...
The nondeterminism boundary is exactly transponder/env.py: the clock (claim leases), the claim files
on disk, and git. transponder/scope.py and transponder/witness.py are pure logic over this membrane.
...
default (TRANSPONDER_FLIGHT=0 to disable), a default that was bought the hard way: the recorder used
to be opt-in, and when the old gate starved a two-session fleet (#4) there was no tape.

Sessions land in TRANSPONDER_FLIGHT_DIR, default `~/.transponder/flight` — absolute, and outside every
...
that imports it — callers import transponder.flight lazily, so an install without the extra still runs
...
def install() -> None:
    """On by default; TRANSPONDER_FLIGHT=0 turns it off."""
